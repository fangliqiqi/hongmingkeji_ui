<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['share:crmClerk:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['share:crmClerk:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['share:crmClerk:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="crmClerkList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="区域" align="center" prop="id" />
...
<script>
import { listCrmClerk, getCrmClerk, delCrmClerk, addCrmClerk, updateCrmClerk, exportCrmClerk,getCrmrelatedList } from "@/api/share/crmClerk";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCrmClerk(this.queryParams).then(response => {
        this.crmClerkList = response.rows;
    ...
    // 根据部门 ID获取相关专员
    getrelatedList(){
      getCrmrelatedList({deptId:153}).then(res=>{
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCrmClerk(this.form).then(response => {
            ...
            addCrmClerk(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除外勤办事员编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delCrmClerk(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有外勤办事员数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportCrmClerk(queryParams);
        }).then(response => {
