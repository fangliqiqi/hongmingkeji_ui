<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['share:channelType:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['share:channelType:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['share:channelType:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['share:channelType:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="channelTypeList" @selection-change="handleSelectionChange">
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="商机类目">
          <el-select v-model="form.categoryType" placeholder="请选择商机类目">
            <el-option v-for="dict in categoryOptions"
              :key="dict.dictValue"
              :label="dict.dictLabel"
              :value="dict.dictValue" 
              ></el-option>
        ...
        <el-form-item label="增值服务" prop="valueAdd">
          <el-input v-model="form.valueAdd" placeholder="请输入增值服务类目" />
        </el-form-item>
...
<script>
import { listChannelType, getChannelType, delChannelType, addChannelType, updateChannelType, exportChannelType,rzCompanyType } from "@/api/share/channelType";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listChannelType(this.queryParams).then(response => {
        this.channelTypeList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getChannelType(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateChannelType(this.form).then(response => {
            ...
            addChannelType(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有商机类型数据项?', "警告", {
          ...
          return exportChannelType(queryParams);
        }).then(response => {
