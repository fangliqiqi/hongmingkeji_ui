<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <!-- <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['share:channelWallet:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['share:channelWallet:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['share:channelWallet:remove']"
        >删除</el-button>
      </el-col> -->
    <!--  <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['share:channelWallet:export']"
        >导出</el-button>
      </el-col>-->
    ...
    <el-table v-loading="loading" :data="channelWalletList" @selection-change="handleSelectionChange" :row-class-name="tableRowClassName" >
      <el-table-column type="selection" width="55" align="center" />
      <!-- <el-table-column label="用户id" align="center" prop="userId" /> -->
      <el-table-column label="公司名称" align="center" prop="companyName" />
      <el-table-column label="账户余额" align="center" prop="amount" />
      <!-- 账户金额最低预警 -->
      <el-table-column label="最低金额预警" align="center"  prop="warningAmount"  :rules='rules'>
...
<script>
import { listChannelWallet, getChannelWallet, delChannelWallet, addChannelWallet, updateChannelWallet, exportChannelWallet,addFundWarning,getCompanyList } from "@/api/share/channelWallet";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listChannelWallet(this.queryParams).then(response => {
        this.channelWalletList = response.rows;
         this.channelWalletList.map((item,index)=>{
    ...
    blurtevent(row){
      ...
      if(newwarningAmount){
       ...
       this.warningForm.amount=row.amount
    ...
    handleUpdate(row) {
      ...
      getChannelWallet(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateChannelWallet(this.form).then(response => {
            ...
            addChannelWallet(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有用户钱包数据项?', "警告", {
          ...
          return exportChannelWallet(queryParams);
        }).then(response => {
