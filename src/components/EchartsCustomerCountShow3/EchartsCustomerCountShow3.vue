<template>
  <div class="bar">
    ...
    <div
      id="EchartsCustomerCountShow3"
      ref="EchartsCustomerCountShow3"
...
import {resourceDetail,getYears} from "../../api/share/customerCountShow";
export default {
  data() {
    name: "EchartsCustomerCountShow3";
    return {
      //传给后台的年份
      Year: [],
      //传给后台的月份
...
getYears().then(response=>{
    this.Time.push(response.data.years)
   })
  },
  ...
  methods: {
    resize() {
      this.EchartsCustomerCountShow3.resize();
    },
     nian(){
     getYears().then(response=>{
    ...
    async getMonth(item){
      ...
      let myChart = echarts.init(
        document.getElementById("EchartsCustomerCountShow3")
      );
      ...
      let fiveWeek = [];
      let sixWeek = [];
      response.data.sourceTypeTotal.forEach(item => {
       

      cateTypeLv.push(this.selectDictLabel(this.cateTypeLvOptions, item.cateTypeLv));
